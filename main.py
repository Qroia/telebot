    ...
    def handler_start(message):
        ...
        with open('welcome.jpg', 'rb') as photo:
            bot.send_photo(
                chat_id=chat_id,
                photo=photo,
                caption='Добро пожаловать {}, user id - {}'.format(message.from_user.first_name,
                                                                    chat_id,),
                ...
                if func.check_balance(chat_id, func.check_price_number(call.data[7:])) == 1:
                    ...
                    try:
                        ...
                        if resp[0] == True:
                            bot.send_message(
                                chat_id=chat_id,
                                text=f'Сервис: {func.get_service_name(call.data[7:])}\nВаш номер: {resp[1]}\nОжидайте прихода смс\n\n<i>Если смс не прийдет через 2 минуты 30 секунд, то вам вернуться потраченные деньги за этот номер, аренда номера будет отменена!!!</i>',
                                parse_mode='html'
                            )
